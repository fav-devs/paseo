...
// Best-effort: reverse-engineers the project directory name that Cursor derives
// from the workspace path. Cursor encodes the path by replacing separators/dots
// with hyphens and stripping leading/trailing hyphens. If Cursor changes this
// encoding, transcript replay silently returns empty (no crash).
...
function mapToolCallToTimeline(
  ...
  // Only shellToolCall is fully mapped; other tools (file edits, search, etc.)
  // fall through to an "unknown" detail. Expand as Cursor exposes more stable schemas.
...
// Note: "agent" is a generic binary name. If the user has another tool named
// "agent" on PATH, isAvailable() will return true even though it is not the
// Cursor CLI. Users can override the binary path via runtimeSettings.command.
